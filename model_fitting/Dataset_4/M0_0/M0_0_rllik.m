function LL = M0_0_rllik(x,subdata,opts)
...
%     
    ...
    Q = w*Qm + (1-w)*Qd(1,:)'+ st.*M;                              % mix TD and model-based values
...
%     LL = LL + b*Q2(subdata.choice2(t)) - logsumexp(b*Q2(:));
    LL = LL + b_2*Qd(state2,subdata.choice2(t)) - logsumexp(b_2*Qd(state2,:));
    logP2 = b_2*Qd(state2,subdata.choice2(t)) - logsumexp(b_2*Qd(state2,:));
    M = [0; 0];
