function LL = M21_3_4_4_rllik(x,subdata,opts)
...
%     logP1=b_1*Q(subdata.choice1(t))-logsumexp(b_1*Q);
    ...
    Q2 = mixing_weight_base_step2* W2(subdata.state2(t),:)'+ (1-mixing_weight_base_step2)*Qd(state2,:)';
...
%     fai=0;
    ...
    if t <= 4
        index=find((subdata.state2(1:t-1)==1) & (subdata.choice2(1:t-1)==1));
        ...
        index=find((subdata.state2(1:t-1)==1) & (subdata.choice2(1:t-1)==2));
        if isnan(mean(reward_4(index)))
            W2(1,2)=0;
        else
            switch length(reward_4(index)')
        ...
        index=find((subdata.state2(1:t-1)==2) & (subdata.choice2(1:t-1)==1));
        if isnan(mean(reward_4(index)))
            W2(2,1)=0;
        else
            switch length(reward_4(index)')
        ...
        index=find((subdata.state2(1:t-1)==2) & (subdata.choice2(1:t-1)==2));
        if isnan(mean(reward_4(index)))
            W2(2,2)=0;
        else
            switch length(reward_4(index)')
