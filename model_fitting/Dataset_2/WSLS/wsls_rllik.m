...
%     isequal(subdata.destinations(92,:),subdata.destinations(91,:))
...
%     if subdata.points(t-1) > 4.5
%         if mod(subdata.choice(t)-1,2)+1 == mod(subdata.choice(t-1)-1,2)+1
%             P = 1 - epi/2;
%         else
%             P = epi/2;
%         end
...
%     end
    ...
    if subdata.points(t-1) > 4.5
        if subdata.destinations(t-1,subdata.choice(t)) == subdata.destinations(t-1,subdata.choice(t-1))
            P = 1 - epi/2;
        else
            P = epi/2;
        end
